...
export default function createEvent(action, nextState, prevState) {
    switch (action.type) {
        ...
        case State.BOARD_RENDERED: {
            const payload = Object.assign(Object.assign({}, getRenderPayload(action.payload)), { renderTime: action.metadata.time });
            return [BOARD_RENDERED, payload];
        }
        case State.BOARD_UPDATED: {
            const payload = nextState.board && action.payload.id === nextState.board.id
                ? getRenderPayload(nextState.board)
                : {};
            return [BOARD_UPDATED, payload];
        }
        case State.BOARD_DELETED: {
            const payload = prevState.board && action.payload === prevState.board.id
...
function getRenderPayload(board) {
    ...
    const gerberZeroSuppression = gerberOptions.zeroSuppression || defaultGerberOptions.zeroSuppression;
    const gerberCoordFormat = gerberOptions.coordinateFormat || defaultGerberOptions.coordinateFormat;
    const drillUnits = drillOptions.units || defaultDrillOptions.units;
    const drillZeroSuppression = drillOptions.zeroSuppression || defaultDrillOptions.zeroSuppression;
    const drillCoordFormat = drillOptions.coordinateFormat || defaultDrillOptions.coordinateFormat;
    const payload = {
        ...
        sourceFilesHash: board.sourceIds
            .reduce((hasher, id) => hasher.update(id), new MD5())
            .digest('hex'),
    };
    return payload;
}
